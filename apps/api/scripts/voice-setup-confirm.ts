          ...
          tools: [
            ...
            toolDef(
              ...
              [
                ...
                '(digits, "номер один", "первый", "#KU-4471", "ка-у четыре четыре семь',
                'один"). Pass the raw phrase verbatim in order_number — the backend',
                'normalizes word forms. RESPONSE shapes:',
                'FOUND: { ok: true, output: { found: true, order_number: string,',
                'plate: string, driver_name: string, pickup: string, delivery: string,',
                'stage_ru: string, stage_ua: string, progress_percent: number, status:',
                'string } }. Use these to compose ONE short status sentence in the',
                'caller language. NOT FOUND: { ok: true, output: { found: false,',
                'message_ru: string, message_ua: string } } — ask the caller to repeat',
                'the number; retry the tool ONCE if needed.',
              ].join(' '),
              publicUrl,
