// Phase 6 D-04 — Backend port of apps/web/src/app/(main)/dashboard/tracking/_lib/polyline-utils.ts.
//
// Identical math (equirectangular approximation). Used by order-ticker.ts to
// interpolate truck position along the OSRM polyline as progress_percent ticks
// 0 → 100. Math byte-equivalent so backend-driven truck position matches the
// web's local truck-marker animation (RESEARCH §D-04).
//
// DO NOT diverge from the web file — port-only. Any future polyline math
// changes should land in BOTH files (regression-test via the snapshot below).
