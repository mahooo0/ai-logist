...
// grammY's InlineKeyboardButton is a union type; cast to access callback_data.
...
describe('telegram-keyboards-phase6', () => {
  describe('loadingKeyboard RU', () => {
    ...
    it('first button text is RU confirm label', () => {
      const kbd = loadingKeyboard('abc-123', 'ru');
      const btn = kbd.inline_keyboard[0]?.[0] as CallbackButton | undefined;
      expect(btn?.text).toBe('✅ Да, подтверждаю');
    });

    it('first button callback_data is confirm_loading:<orderId>', () => {
      const kbd = loadingKeyboard('abc-123', 'ru');
      const btn = kbd.inline_keyboard[0]?.[0] as CallbackButton | undefined;
      expect(btn?.callback_data).toBe('confirm_loading:abc-123');
    });

    it('second button text is RU decline label', () => {
      const kbd = loadingKeyboard('abc-123', 'ru');
      const btn = kbd.inline_keyboard[1]?.[0] as CallbackButton | undefined;
      expect(btn?.text).toBe('⚠️ Нет, есть проблема');
    });

    it('second button callback_data is decline_loading:<orderId>', () => {
      const kbd = loadingKeyboard('abc-123', 'ru');
      const btn = kbd.inline_keyboard[1]?.[0] as CallbackButton | undefined;
      expect(btn?.callback_data).toBe('decline_loading:abc-123');
    });
  });
  ...
  describe('loadingKeyboard UA', () => {
    it('first button text is UA confirm label', () => {
      ...
      const btn = kbd.inline_keyboard[0]?.[0] as CallbackButton | undefined;
      expect(btn?.text).toBe('✅ Так, підтверджую');
    });

    it('second button text is UA decline label', () => {
      const kbd = loadingKeyboard('abc-123', 'ua');
      const btn = kbd.inline_keyboard[1]?.[0] as CallbackButton | undefined;
      expect(btn?.text).toBe('⚠️ Ні, є проблема');
    });

    it('callback_data uses orderId regardless of lang', () => {
      const kbd = loadingKeyboard('xyz-999', 'ua');
      const btn0 = kbd.inline_keyboard[0]?.[0] as CallbackButton | undefined;
      const btn1 = kbd.inline_keyboard[1]?.[0] as CallbackButton | undefined;
      expect(btn0?.callback_data).toBe('confirm_loading:xyz-999');
      expect(btn1?.callback_data).toBe('decline_loading:xyz-999');
    });
  });
  ...
  describe('deliveryKeyboard RU', () => {
    ...
    it('first button callback_data is confirm_delivery:<orderId>', () => {
      const kbd = deliveryKeyboard('order-uuid', 'ru');
      const btn = kbd.inline_keyboard[0]?.[0] as CallbackButton | undefined;
      expect(btn?.callback_data).toBe('confirm_delivery:order-uuid');
    });

    it('second button callback_data is decline_delivery:<orderId>', () => {
      const kbd = deliveryKeyboard('order-uuid', 'ru');
      const btn = kbd.inline_keyboard[1]?.[0] as CallbackButton | undefined;
      expect(btn?.callback_data).toBe('decline_delivery:order-uuid');
    });
  });
  ...
  describe('deliveryKeyboard UA', () => {
    it('first button text is UA confirm label', () => {
      ...
      const btn = kbd.inline_keyboard[0]?.[0] as CallbackButton | undefined;
      expect(btn?.text).toBe('✅ Так, отримав');
    });
