...
**Domain:** Logistics dispatching backend (Telegram + voice intake → LLM extraction → PostGIS truck matching → deterministic pricing → live GPS tracking) + Next.js admin demo
...
**Backend language: Node.js 22 LTS + TypeScript 5.7 strict.**

Rationale (no hedging):

1. **Frontend is already TypeScript** (locked by spec §7.1: Next.js 16 + React 19 + TS + Zod). Same language = shared `zod` schemas for `extractRequest` tool output, shared types for REST/WS DTOs, one `pnpm` workspace, one set of devs.
2. **Drizzle ORM has first-class PostGIS support** with an official guide showing exactly the `geometry('point')` column, GiST index, and `<->` KNN operator the spec §4.2 requires. No SQLAlchemy GeoAlchemy2 boilerplate needed.
3. **Anthropic SDK for TypeScript ships a `betaZodTool` + `toolRunner` helper** (`@anthropic-ai/sdk/helpers/beta/zod`) that takes a Zod schema and runs the function-calling loop for you. The Python equivalent requires manually wiring `tool_use_id` echoes. TS path is shorter for the §4.1 `extractRequest` tool.
4. **grammY** is a modern, TypeScript-native Telegram framework with explicit webhook support and 1.43.x stable. aiogram is excellent but it ties us to Python. Since (1)–(3) push Node, grammY drops in naturally.
5. **BullMQ + ioredis** is the proven Node combo for the §4.7 tracking loop (`setInterval` polls Wialon / driver app, debounce, push WS). TypeScript-first, written by the same team behind Bull.

**Python (FastAPI + aiogram + SQLAlchemy + GeoAlchemy2 + Anthropic Python SDK) is viable** and slightly better only if the team is Python-heavy. Given the spec mandates TS on the front and the team works in Claude Code (which speaks TS fluently), **Node wins**.

Voice is deferred (§5.2 marks it optional), so the Python-LLM-ecosystem advantage doesn't apply — when voice is added later, ElevenLabs Agents SDK has both Python and TS bindings, so it's not a lock-in.
...
pnpm add fastify @fastify/websocket pino \
...
pnpm add -D typescript tsx vitest @types/node @types/pg \
  testcontainers @biomejs/biome pino-pretty
...
services:
  ...
  backend:
    ...
    depends_on: [db, redis]
    environment:
      DATABASE_URL: postgres://ailogist:${DB_PASSWORD}@db:5432/ailogist
  ...
  admin:
    build: ./apps/admin
    depends_on: [backend]
  # Optional for demo (use public Nominatim + OSRM demo servers initially)
  # osrm:
...
**Total demo monthly cost:** ~$6 VPS + LLM token spend.
...
**If the team picks Python instead of Node** (decision rests with the team):
...
**If the demo expands to true production scale:**
...
**If we add the voice channel (§5.2) post-demo:**
...
**Context7-equivalent / Official docs (HIGH confidence):**
...
**Comparison sources (MEDIUM confidence, multiple corroborating):**
...
**Python alternative path (MEDIUM confidence):**
...
*Stack research for: logistics dispatching backend + Next.js admin (AI-Логист)*
*Researched: 2026-06-08*
